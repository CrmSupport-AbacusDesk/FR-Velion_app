...
app.controller('SFA_BM', function ($scope, $rootScope, searchSelect, $ionicModal, $state, myRequestDBService, myAllSharedService, $ionicLoading, $cordovaSQLite, $ionicPopup, $timeout, $ionicActionSheet, $cordovaImagePicker, Camera, $cordovaFileTransfer, $cordovaGeolocation, $cordovaToast, $location, $window, $ionicPlatform, $ionicHistory, $ionicModal, $stateParams, $ionicScrollDelegate, $ionicPopover) {
    ...
    if ($location.path() == '/tab/sfa_billing_details') {
        ...
        .then(function (result) {
            ...
            $scope.sfa_billing_details = result.data;
            console.log($scope.sfa_billing_details);
    ...
    $scope.limit;
    $scope.temp_billinglist=[];
    $scope.billinglist=[];

    $scope.billinglistdata = function (targetSRC,searchkey) {
        ...
        if(targetSRC=='search'){
            ...
            $scope.billinglist=[];
            console.log($scope.billinglist.length);
        }
        $scope.limit = $scope.billinglist.length;
        console.log($scope.billinglist.length);
        fetchingRecords = true;
        $ionicLoading.show({
            template: '<p>Loading...</p><ion-spinner icon="android"></ion-spinner>'
        });
        myRequestDBService.orpPostServiceRequest('/invoice/getInvoiceList', { 'dr_code': $scope.data.dr_code ,'limit': $scope.limit ,'search' : $scope.data.search ,'page':$scope.page})
        .then(function (result) {
            console.log(result);
            // $scope.temp_billinglist = result.data;
            if(!$scope.temp_billinglist || $scope.temp_billinglist==0){
                $scope.noMoreListingAvailable=true;
            }

            // $scope.billinglist =$scope.billinglist.concat($scope.temp_billinglist);
    ...
    if ($location.path() == '/tab/sfa_billing_list') {
        ...
        {
            $scope.data.dr_code = myAllSharedService.drTypeFilterData.dr_code;
        }
        else{
            $scope.data.dr_code = 0;
        }
    ...
    $scope.getOrderListData = function (targetSRC) {
        ...
        // console.log(myAllSharedService.drTypeFilterData.detail.Pend_orderdetail);
        ...
        myRequestDBService.orpPostServiceRequest('/invoice/getPendingOrderList', { 'dr_code': $scope.data.dr_code ,'page':$scope.page,'search':$scope.data.search})
        .then(function (response) {
            $ionicLoading.hide();
            console.log(response);
    ...
    if ($location.path() == '/tab/sfa-order-list') {
        ...
        if(myAllSharedService.drTypeFilterData.detail.Pend_orderdetail == 'visited'){
            $scope.page = myAllSharedService.drTypeFilterData.detail.order_page;
        }else{
            $scope.page = 0;
        }
    ...
    $scope.getOrderDetailData = function () {
        ...
        myRequestDBService.orpPostServiceRequest('/invoice/getPendingOrderDetail', $scope.data.orderId).then(function (response) {
            ...
            // myAllSharedService.drTypeFilterData.detailpage = '';
    ...
    if ($location.path() == '/tab/sfa_order_detail') {

        $scope.data.orderId = myAllSharedService.drTypeFilterData.orderId;
        myAllSharedService.drTypeFilterData.detail.Pend_orderdetail = 'visited';
        console.log(myAllSharedService.drTypeFilterData.detailpage);
        $scope.getOrderDetailData();
    ...
    $scope.onGoToOrderDetailPage = function (orderId) {
        myAllSharedService.drTypeFilterData.orderId = orderId;
        myAllSharedService.drTypeFilterData.detail.order_page = $scope.page;
        $state.go('tab.sfa_order_detail');
    }
    ...
    $scope.onGoToBillingDetailPage = function (orderId) {
        ...
        $state.go('tab.sfa_billing_details');
    }
    ...
    $scope.onModifyTypeHandler = function (type) {
        ...
        $scope.getOrderListData('onLoad');
        console.log('onModifyTypeHandler');
    }

    $scope.activeFilter = function(value)
    {
        console.log("Test");
    ...
    $scope.downloadPdf = function (pdfName, title) {
        console.log('step 1');
        var pdf_url = "http://microtek.abacusdesk.com/uploads/" + pdfName;
        ...
        $scope.downloadAccountStatement = function () {
            ...
            myRequestDBService.orpPostServiceRequest('/Dashboard_Controller/GENRATE_PDF', $scope.dash_filter).then(function (result) {
                ...
                $scope.downloadPdf("account-statement.pdf",'Account Statement');
                // }
            }, function (err) {
                $ionicLoading.hide();
                $ionicPopup.alert({
        ...
        $scope.downloadOutstandingReport = function () {

            $ionicLoading.show({
                template: '<ion-spinner icon="android"></ion-spinner><p>Loading...</p>'
            });
            myRequestDBService.orpPostServiceRequest('/Dashboard_Controller/OUTSTANDING_REPORT_PDF','').then(function (result) {
                console.log(result);
                $ionicLoading.hide();
                if(result.status == 'Success')
                {

                $scope.downloadPdf("Outstanding_Report.pdf",'Outstanding');
                }
            }, function (err) {
                $ionicLoading.hide();
                $ionicPopup.alert({
        ...
        if ($location.path() == '/tab/orp_account_statement') {
            $scope.dash_filter.type = 'Transaction';
            $scope.dash_filter.transaction = 'All';
            $scope.dash_filter.drCode = myAllSharedService.drTypeFilterData.dr_code;

            $scope.GET_ACCOUNT_STATEMENT();
